...
public:
    ...
    void string(std::string_view s) {
        json += "\"";
        // Minimal escaping for performance - expand if needed for strict JSON
        for (char c : s) {
            if (c == '"') json += "\\\"";
            else if (c == '\\') json += "\\\\";
            else if (c == '\n') json += "\\n";
...
void dfs_cycle(int u, const OrthogonalGraph& graph, std::vector<uint8_t>& visited, std::vector<int>& stack, std::vector<std::vector<GraphNode>>& cycles) {
    ...
    int edgeIndex = graph.vertices[u].firstOut;
    while (edgeIndex != -1) {
        const OGEdge& edge = graph.edges[edgeIndex];
        ...
        if (visited[v] == 1) {
            ...
            dfs_cycle(v, graph, visited, stack, cycles);
        }
...
std::vector<std::vector<GraphNode>> DetectCycles(const OrthogonalGraph& graph) {
    ...
    for (size_t i = 0; i < graph.vertices.size(); ++i) {
        if (visited[i] == 0) {
            dfs_cycle((int)i, graph, visited, stack, cycles);
        }
    }
    return cycles;
}
