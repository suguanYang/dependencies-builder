...
export class ConnectionScheduler {
    ...
    public async scheduleConnectionAutoCreate(isManualTrigger: boolean = false) {
        try {
            ...
            if (existingAction) {
                ...
                if (existingAction.status === 'pending') {
                    if (isManualTrigger) {
                        ...
                        if (lockedAction) {
                            info(`Advanced scheduled connection auto-create task (Action ID: ${existingAction.id}) to now`)
                            ...
                            info(`Failed to lock action ${existingAction.id} for manual trigger. It may be already running.`)
                            return existingAction
                        }
                    } else {
                        // If auto trigger (node change), we debounce (do nothing if already pending)
                        info(`Connection auto-create already scheduled (Action ID: ${existingAction.id}). Debouncing.`)
                        return existingAction
                    }
                }
            }
            ...
            info(`Scheduled connection auto-create task (Action ID: ${action.id}) for ${scheduledFor.toISOString()}`)
