            ...
            {isOpen && (
                <div className="border-t bg-white p-4">
                    {isLoading ? (
                        ...
                        <div className="text-gray-500 text-sm italic">No explicit file connections found (might be implicit).</div>
                    ) : (
                        <div className="space-y-3">
                            <div className="flex justify-between items-center pb-2 border-b">
                                <span className="text-xs font-semibold uppercase text-gray-500">Connections ({data?.total})</span>
                            </div>
                            <div className="space-y-2">
                                {data?.data.map((conn, i) => {
                                    const fromNodeLink = generatePermanentLink(conn.fromNode, conn.fromNode.project?.addr)
                                        ...
                                        <div key={`${conn.id}-${i}`} className="flex items-start gap-3 text-sm group">
                                            ...
                                            <div className="flex-1 min-w-0 space-y-1">
                                                <div className="flex items-center gap-2 flex-wrap">
                                                    ...
                                                    <Badge variant="outline" className="text-[10px] h-4 px-1">{conn.fromNode.type}</Badge>
                                                </div>
...
export function CyclesList({ cycles, itemHeight = 80, isLoading, error, projectId, onCycleClick }: CyclesListProps) {
