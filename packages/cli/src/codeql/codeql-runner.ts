...
export class CodeQL {
  ...
  private static readonly EXECUTABLE_PATH =
    process.env.CODEQL_EXCUTABLE_PATH || '/usr/local/codeql/codeql'
  ...
  constructor() {
    ...
    this.results = path.join(
      this.databasePath,
      'results',
      projectNameToCodeQLName(ctx.getMetadata().name),
    )
  ...
  async initialize(): Promise<void> {
    ...
    try {
      const versionString = await run(
        CodeQL.EXECUTABLE_PATH,
        ['version', '--format=json'],
        {},
        true,
      )
    ...
    try {
      const languagesString = await run(
        CodeQL.EXECUTABLE_PATH,
        [
          'resolve',
          'languages',
  ...
  async decodeSingleResult<T = any>(queryName: string): Promise<T[]> {
    ...
    try {
      ...
      const result = JSON.parse(
        readFileSync(path.join(this.outputPath, `${queryName}.json`)).toString(),
      )
